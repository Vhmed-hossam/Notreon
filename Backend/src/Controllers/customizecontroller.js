...
export async function ChangeName(req, res) {
    ...
    try {
        ...
        user.name = newName;
        await user.save();
        return res.status(200).json({ message: SuccessMessages.nameChanged, user });
    } catch (error) {
...
export async function ChangeProfilePic(req, res) {
    ...
    try {
        if (!newProfilepic.trim()) {
            return res.status(400).json({ error: ErrorMessages.requirednewprofilepic })
        }
        ...
        user.profilePic = newImage.url;
        user.profilePicId = newImage.fileId;
        await user.save();
